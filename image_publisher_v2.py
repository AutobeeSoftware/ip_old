...
def image_publisher():
    ...
    out = cv2.VideoWriter("test3.avi",fourcc,10.0,((width*3, height)))
    ...
    pub2 = rospy.Publisher("herb_angle", Float32, queue_size=1)
    msg = herb_location()
    ...
    while not rospy.is_shutdown():
        ...
        if obj_loc != None:
            ...
            (x,y), (w,h), tag = nearest
            # msg_ = Float32((cx - (width/2)) / (width/2))
            msg_ = Float32((float(cx) - float(width / 2.0)) / float(width / 2.0))
            pub2.publish(msg_)
